...
vhosts=( "default.conf" "default-ssl.conf" "redirect-http-https.conf" "redirect-from-to-http.conf" "redirect-from-to-https.conf" )
for vhost in "${vhosts[@]}"
do
    ...
    sed -i "s!\${NGINX_VHOST_DOCUMENT_ROOT}!${NGINX_VHOST_DOCUMENT_ROOT}!" /etc/nginx/conf.d/$vhost
    sed -i "s/\${NGINX_VHOST_REDIRECT_FROM_SERVER_NAME}/${NGINX_VHOST_REDIRECT_FROM_SERVER_NAME}/" /etc/nginx/conf.d/$vhost
    sed -i "s/\${NGINX_VHOST_REDIRECT_TO_PROTOCOL}/${NGINX_VHOST_REDIRECT_TO_PROTOCOL}/" /etc/nginx/conf.d/$vhost
    sed -i "s/\${NGINX_VHOST_REDIRECT_TO_SERVER_NAME}/${NGINX_VHOST_REDIRECT_TO_SERVER_NAME}/" /etc/nginx/conf.d/$vhost
done
...
sed -i "s/\${NGINX_VHOST_DNS_RESOLVER_IP}/${NGINX_VHOST_DNS_RESOLVER_IP}/" /etc/nginx/includes/loc-echo.conf
sed -i "s/\${NGINX_VHOST_DNS_RESOLVER_IP}/${NGINX_VHOST_DNS_RESOLVER_IP}/" /etc/nginx/includes/loc-minio.conf
